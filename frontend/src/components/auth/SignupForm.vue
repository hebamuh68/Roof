      ...
      <div class="relative bg-white bg-opacity-5 backdrop-blur-xl rounded-2xl sm:rounded-3xl shadow-2xl p-4 sm:p-6 lg:p-8 border border-white border-opacity-10">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            <div>
              ...
              <input
                ...
                required
                :class="[
                  'w-full rounded-lg sm:rounded-xl border-2 bg-gray-900 bg-opacity-40 backdrop-blur-sm px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-base text-white placeholder-gray-400 focus:outline-none transition-all duration-300',
                  v$.first_name.$error ? 'border-red-500' : 'border-gray-600 border-opacity-40 focus:border-green-500'
                ]"
                ...
                placeholder="John"
              />
              <p v-if="v$.first_name.$error" class="text-red-400 text-xs mt-1">
        ...
        <div class="mt-4 sm:mt-6 text-center">
          <p class="text-gray-300 text-xs sm:text-sm">
            ...
            <a href="#" class="font-semibold hover:text-white transition-colors" style="color: #4BC974;">
              Sign In
            </a>
          </p>
        </div>
...
import { createEmptyUserData } from "../../types/userType";
...
const handleSignup = async () => {
  ...
  try {
    ...
    Object.keys(formData).forEach((key) => (formData[key as keyof userData] = Array.isArray(formData[key as keyof userData]) ? [] : ""));
  } catch (error: any) {
